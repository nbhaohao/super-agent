...
function systemPrompt(): string {
  try {
    return '你是有工具能力的助手，需要数据/操作时调用工具，不要编造。' + registry.getDeferredToolSummary();
  } catch {
...
function ask() {
  rl.question('\nYou: ', async (input) => {
    ...
    const r = await agentLoop(model, registry.toAISDKFormat(), messages, { system: systemPrompt(), logger });
...
async function main() {
  const mcp = await registry.registerMCPServer('github', new MockMCPClient()).catch(() => [] as string[]); // s8 未实现则跳过
  if (mcp.length) console.log(`已注册 ${mcp.length} 个 Mock MCP 工具`);
  console.log(`m02 手动冒烟 — 共 ${registry.getAll().length} 工具，活跃 ${registry.getActiveTools().length}。自己敲 prompt，exit 退出。`);
  ask();
}
main();
